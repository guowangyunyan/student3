                    ...
                    <section class="panel">
                        <div class="panel-body">
                               ...
                               <select data-placeholder="选择优惠促销类型..." class="chosen-select-promotionType form-control m-bot15 "  >
                                      <c:choose>
										    <c:when test="${!empty promotion && !empty promotion.promotionType }">
										        <option value="1" <c:if test='${promotion.promotionType == 1}'>selected</c:if> >优惠促销</option>
										        <option value="2" <c:if test='${promotion.promotionType == 2}'>selected</c:if> >欢乐时光</option>
										        <option value="3" <c:if test='${promotion.promotionType == 3}'>selected</c:if> >女士之夜</option>
										        <option value="4" <c:if test='${promotion.promotionType == 4}'>selected</c:if> >开怀畅饮</option>
										        <option value="5" <c:if test='${promotion.promotionType == 5}'>selected</c:if> >特别活动</option>
										        <option value="6" <c:if test='${promotion.promotionType == 6}'>selected</c:if> >精选套餐</option>
										    </c:when>
										    <c:otherwise>
                          ...
                          <input type="hidden"  id="promotionId" value="<c:if test="${!empty promotion.promotionId}">${promotion.promotionId}</c:if>" />
                          <input type="hidden"  id="restaurantId" value="<c:if test="${!empty restaurantInfo && !empty restaurantInfo.restaurantId}">${restaurantInfo.restaurantId}</c:if>" />
                                <div class="form-group">
                                    ...
                                    <input type="input" name='promotionName' class="form-control" id="promotionName" value="<c:if test='${!empty promotion && !empty promotion.promotionName}'>${promotion.promotionName}</c:if>" />
                                </div>
                                
                                <div class="form-group">
                                    <label class="font16 font-bold">促销开始时间 </label>
                                    <input size="16" type="text"   id="startDate" readonly="" class="form-control" placeholder="选择时间" size="16"  value="<c:if test='${!empty promotion && !empty promotion.startDate}'>${promotion.startDate}</c:if>">
                                </div>
                                <div class="form-group">
                                    <label class="font16 font-bold">促销结束时间 </label>
                                    <input size="16" type="text"   id="endDate"  readonly="" class="form-control" placeholder="选择时间" size="16"  value="<c:if test='${!empty promotion && !empty promotion.endDate}'>${promotion.endDate}</c:if>">
                                </div>


                               <div class="form-group">
								<label class="font16 font-bold">促销内容</label>
								<textarea class="form-control"  onkeyup="calNum()"  style="height:150px;resize:none" name="content" id="content"   rows=4 maxlength="1000"><c:if test='${!empty promotion && !empty promotion.content}'>${promotion.content}</c:if></textarea>
                               ...
                               <div class="form-group" >
                               	...
                               	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                               	<input type="checkbox" id="isActive" <c:if test="${!empty promotion && promotion.isActive==1 }">checked</c:if> data-toggle="switch" />
                             </div>
